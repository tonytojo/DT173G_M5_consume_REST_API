...
if (devMode)
   ...
   API = "https://studenter.miun.se/~tojo8500/dt173g/moment5/courses.php"; //address to server REST-API
   ...
   fetch(API)
      ...
      .then(data =>
      {
         if (data.message !== "No courses found")
         {
            ...
            data.forEach(item =>
            {
               coursesEl.innerHTML += `<tr contenteditable='true' id=${item.id}> <td class="code">${item.code}</td><td class="name">${item.name}</td><td class="progression">${item.progression}</td><td contenteditable='false' class="plan"><a target="_blank" href=${item.plan}>Till kursplanen</a></td> <td class="delete"><button class="btn" onclick="deleteCource(${item.id})">Delete</button></td> <td class="update"><button class="btn" onclick="updateCource(${item.id})">Update</button></td></tr>`;
            });
         }
         else
         {
...
{
   return $('#code').val() === "" || $('#name').val() === "" || $('#progression').val() === "" || $('#plan').val() === "" ? false : true;
}
